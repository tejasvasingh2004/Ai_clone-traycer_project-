  ...
  try {
    ...
    for (const imp of imports) {
      ...
      if (imp.startsWith('.') || imp.startsWith('/')) {
        ...
        const extensions = ['.ts', '', '.tsx', '.jsx', '/index.ts', '/index'];
        let resolved = false;
 ...
 */
export async function verifyCode(projectRoot: string = process.cwd()): Promise<VerificationResult> {
  ...
  try {
    ...
    const tsconfigExists = await fileExists(join(projectRoot, 'tsconfig.json'));
    ...
    const eslintrcExists = await fileExists(join(projectRoot, '.eslintrc.json'));
    const eslintConfigExists = await fileExists(join(projectRoot, 'eslint.config'));
    ...
    try {
      ...
      for (const entry of indexData) {
        const importErrors = await checkImportResolution(entry.filePath, projectRoot);
