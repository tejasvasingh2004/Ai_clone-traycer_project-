...
export function setupTerminalWebSocket(server: HttpServer): WebSocketServer {
  ...
  wss.on('connection', (ws: WebSocket, request) => {
    ...
    // Default to workspace root (traycer-web parent directory) if no repo specified
    // HLD §2: files live in `repositories/<id>/` at the project root.
    // traycer-web is in the project root, so process.cwd() is projectRoot/traycer-web
